...
if __name__ == "__main__":
    ...
    experimentConfigSampleIndex = [i for i in range(21)]   # 分解成多少条IMF 就写几
    ...
    for sampleIndex in experimentConfigSampleIndex:
        ...
        #featureIndex = np.array([9,8,7,6,5,4,3,2,1])
        if sampleIndex == 0:
            featureIndex = np.array([9,8,7,6,5,4,3,2,1,0])  # 1
        elif sampleIndex == 1:
            featureIndex = np.array([9,8,7,6,5,4,3,2,1,0])  # 2
        elif sampleIndex == 2:
            featureIndex = np.array([9,8,7,6,5,4,3,2,1,0])  # 3
        else:
            featureIndex = np.array([9,8,7,6,5,4,3,2,1,0])  # 4
        dataset = Dataset(series, featureIndex, 0.8)

        parameter = dict()

        parameter['nodeNums'] = [25]
        parameter['lossName'] = 'mse'
        parameter['optimizer'] = 'adam'
        parameter['batchSize'] = 100
        parameter['epochs'] = 765
